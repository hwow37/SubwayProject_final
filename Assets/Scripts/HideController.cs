...
public class HideController : MonoBehaviour
{
    ...
    public GameObject MoveController;       //이동 회전 물체 잡기 트리거, 충전기 UI
    ...
    void ShowRotateController()     //이동에 성공하면 회전 UI출력,회전에 성공하면 UI숨김
    {
        ...
        if (((ControllerX = RotateControllerState.x) >= 0.8) || ((ControllerX = RotateControllerState.x) <= -0.8))
        {
            Audioplayer.PlayOneShot(CorrectSound);
            RotateController.SetActive(false);
            Invoke("ShowGripController", 2.0f);     //물체잡기 UI 팝업
            limit2 = true;
        }
        if (((ControllerY = RotateControllerState.y) >= 0.8) || ((ControllerY = RotateControllerState.y) <= -0.8))
        {
            Audioplayer.PlayOneShot(CorrectSound);
            RotateController.SetActive(false);
            Invoke("ShowGripController", 2.0f);
            limit2 = true;
        }
    }
    ...
    void ShowGripController()              //물체 잡기 UI 처리함수
    {
        GripController.SetActive(true);
        GameObject.Find("Gripcube").GetComponent<Flicker>().enabled = true;
        GameObject.Find("GripText").SetActive(true);
    }
